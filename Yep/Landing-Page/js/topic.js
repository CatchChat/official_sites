var base64Prefix, delay, isChinese, modulate, msg_pswpItems, pswpItems, topic_pswpItems, viewImage;
...
viewImage = function(pswpItems, pswpIndex, thumbnailElements) {
  var pswpElement, pswpOptions, topc_gallery;
  pswpElement = $('.pswp')[0];
  pswpOptions = {
    index: pswpIndex,
...
$(function() {
  ...
  $.getJSON(url + param, function(response) {
    var audio_duration, audio_element, circle, content, element, i, j, kind, len, len1, map, messages, msg, msg_attachment, msg_img_metadata, msg_img_thumbnail_blur, msg_metadata, ref, repo, shot, topic, topic_attachment, topic_metadata, topic_pswpItem, topic_thumbnail;
    ...
    switch (kind) {
      case "image":
        ...
        $(".topic .images .thumbnails .thumbnail").on("tap", function() {
          return viewImage(topic_pswpItems, $(this).index(), $(".topic .images .thumbnails .thumbnail"));
        });
        break;
      case "dribbble":
        ...
        $(".topic .dribbble a.link").html(shot.title).attr("href", shot.url).attr("target", "_blank");
        pswpItems = [
          {
            msrc: shot.media_url,
        ...
        $(".topic .dribbble .shot").on("tap", function() {
          return viewImage(pswpItems, 0, $(this));
        });
        break;
      case "github":
        repo = topic.attachments[0];
        $(".topic .github").css("display", "table").attr("href", repo.url).attr("target", "_blank");
    ...
    $(".chat .bubble .image").on("tap", function() {
      return viewImage(msg_pswpItems, $(".chat .bubble .image").index($(this)), $(".chat .bubble .image"));
    });
